...
var updateBlocks = setInterval(function () {
  ...
  if (blockLastTop < 400 || counter == 0) {
    ...
    block.style.top = blockLastTop + 70 + "px";
  ...
  for (var i = 0; i < currentBlocks.length; i++) {
    ...
    var blockTopI = parseFloat(
      window.getComputedStyle(iblock).getPropertyValue("top")
    );
    var holeLeftI = parseInt(
      window.getComputedStyle(ihole).getPropertyValue("left")
    );
    var holeTopI = parseInt(
      window.getComputedStyle(ihole).getPropertyValue("top")
    );
